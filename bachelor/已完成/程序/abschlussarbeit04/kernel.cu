...
void cudaMatrixMul()
{
	...
	Muld<<<dimGrid, dimBlock>>>(d_A, d_B, wA, wB, d_C);
	cudaDeviceSynchronize();

	cudaEventRecord(start, 0);//tick

	Muld<<<dimGrid, dimBlock>>>(d_A, d_B, wA, wB, d_C);
	cudaDeviceSynchronize();
	Muld<<<dimGrid, dimBlock>>>(d_C, d_B, wA, wB, d_D);
	cudaDeviceSynchronize();
...
bool separate(unsigned short *group, unsigned short rts)
{
	short t = -1;
	unsigned short *q;
	unsigned short head = 0, tail = 0;
	...
	cudaMemcpy(inlgroup2, d_inlgroup2, (num_deal1+1)*NOV*sizeof(bool), cudaMemcpyDeviceToHost);
...
void cudaSeparate()
{
	...
	fprintf(fp, "Laufzeit(ms): %0.2f", elapsedTime);
